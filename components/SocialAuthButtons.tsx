    ...
    <View className="gap-3">
      {providers.map(({ id, label, Icon }) => {
        ...
        return (
          <TouchableOpacity
            key={id}
            className="flex-row items-center justify-center gap-3 rounded-full border border-ink-200 bg-white py-3"
            onPress={() => onPress(id)}
      ...
      {error ? (
        <View className="rounded-[10px] border border-coral-200 bg-coral-50 p-2.5">
          <Text className="text-center text-[13px] text-coral-800">{error}</Text>
        </View>
      ) : null}
    </View>
  );
